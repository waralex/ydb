...
namespace {
    ...
    public:
        struct UnitResult {
            ...
            size_t Requests;
            UnitResult() = default;
            UnitResult(uint64_t microseconds, size_t serializedBytes, size_t payloadBytes, size_t requests) 
                : Microseconds(microseconds), SerializedBytes(serializedBytes), PayloadBytes(payloadBytes), Requests(requests) {}
            UnitResult(TDuration duration, size_t serializedBytes, size_t payloadBytes, size_t requests) 
                : Microseconds(duration.MicroSeconds()), SerializedBytes(serializedBytes), PayloadBytes(payloadBytes), Requests(requests) {}
        };
    private:
    ...
    public:
        ...
        TBenchmarkStats GetRpsStats() const {
            return GetStats([](const UnitResult& res) {
                    return static_cast<double>(res.Requests) * 1000000. / res.Microseconds;
                });
        }
        TBenchmarkStats GetSerializeThroughputStats() const {
            return GetStats([](const UnitResult& res) {
                    return static_cast<double>(res.SerializedBytes) * 1000000. / res.Microseconds;
                }, SF_BYTES);
        }
        TBenchmarkStats GetPayloadThroughputStats() const {
            return GetStats([&](const UnitResult& res) {
                    return static_cast<double>(res.PayloadBytes) * 1000000. / res.Microseconds;
                }, SF_BYTES);
        }
    ...
    private:
        template<class Fn>
        TBenchmarkStats GetStats(Fn statsFn, ESizeFormat fmt = SF_QUANTITY) const {
            TBenchmarkStats res(fmt);
            auto view = std::views::transform(Results, statsFn);
            res.Avg = std::accumulate(view.begin(), view.end(), 0.0) / Results.size();
    ...
    class TSerializationBenchmark {
        ...
        size_t PayloadSize;
        public:
            TSerializationBenchmark(size_t countInUnit, size_t unitsCount, size_t payloadSize) : CountInUnit(countInUnit), UnitsCount(unitsCount), PayloadSize(payloadSize) {
            }
...
Y_UNIT_TEST_SUITE(TEvVPutSerializationTest) {
    Y_UNIT_TEST(Base) {
        {
        ...
        /* {
            Cerr << "|||||||||||||||||||||||||" << Endl;
            Cerr << "128 bytes of payload" << Endl;
            TSerializationBenchmark bench(10000, 200,128);
            bench.Run();
        }
        {
            Cerr << "|||||||||||||||||||||||||" << Endl;
            Cerr << "4096 bytes of payload" << Endl;
            TSerializationBenchmark bench(10000, 200,4096);
            bench.Run();
        } */
    }
}
