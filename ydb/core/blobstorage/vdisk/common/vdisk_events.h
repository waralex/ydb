...
#include <ydb/core/blobstorage/vdisk/common/ev_v_put_record.h>
...
namespace NKikimr {
    ...
    struct TEvBlobStorage::TEvVPutBS
        : TEventBS<TEvBlobStorage::TEvVPutBS, BSRecords::TEvVPutBS, TEvBlobStorage::EvVPut>
        , TEventWithRelevanceTracker
    {
        // In current realization it is intentionaly lost on event serialization since
        // LWTrace doesn't support distributed shuttels yet
        ...
        TEvVPutBS(const TLogoBlobID &logoBlobId, TRope buffer, const TVDiskID &vdisk,
                const bool ignoreBlock, const ui64 *cookie, TInstant deadline,
                NKikimrBlobStorage::EPutHandleClass cls)
        {
            InitWithoutBuffer(logoBlobId, vdisk, ignoreBlock, cookie, deadline, cls);
                ...
                const ui64 *cookie, TInstant deadline, NKikimrBlobStorage::EPutHandleClass cls)
        {
            ...
            const ui64* raw = logoBlobId.GetRaw();
            Record.BlobID.RawX1 = raw[0];
            Record.BlobID.RawX2 = raw[1];
            Record.BlobID.RawX3 = raw[2];
            Record.FullDataSize = logoBlobId.BlobSize();

            Record.VDiskID.GroupID = vdisk.GroupID;
            Record.VDiskID.GroupGeneration = vdisk.GroupGeneration;
            /* Record.VDiskID.FailRealm = vdisk.FailRealm;
            Record.VDiskID.FailDomain = vdisk.FailDomain; */
            Record.VDiskID.VDisk = vdisk.VDisk;

            Record.IgnoreBlock = ignoreBlock;

            if (cookie) {
                Record.Cookie = *cookie;
            }
            if (deadline != TInstant::Max()) {
                Record.MsgQoS.DeadlineSeconds = (ui32)deadline.Seconds();
            }
            Record.HandleClass = cls;
            Record.MsgQoS.ExtQueueId = HandleClassToQueueId(cls);
        }
